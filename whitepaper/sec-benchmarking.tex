...
% 
This band extends from 
the mean of the smallest result minus its error 
to the mean of the largest result plus its error, 
and includes all results listed in 
Tab.~\ref{tab:unpolLQCDstatus1} with two or more flavours of sea quarks.
%
...
%-------------------------------------------------------------------------------
\begin{table}[!t]
% Rescale CT14 errors by 1/1.65 to convert from  90\%-CL bands to  68\%-CL bands.
\centering
\renewcommand{\arraystretch}{1.2}
...
As outlined in Sect.~\ref{sec:polPDFs}, polarised PDFs cannot be determined in 
a global QCD analysis with the same accuracy as their unpolarised counterparts.
%
Also, because polarised PDFs do not enter precision physics studies at the LHC, 
...
\caption{\small A comparison of the unpolarized benchmark PDF first moments 
...
the lattice values (right) at $\mu^2=Q^2=4\mbox{ GeV}^2$.}
\label{fig:Bmomsunp}
\end{figure} 
%-------------------------------------------------------------------------------
...
the experimental value of $g_A^{\mathrm{exp}} = 1.2723(23)$~\cite{Olive:2016xmw} 
