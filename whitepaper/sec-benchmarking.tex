...
and define the criteria to appraize the determinations
...
characterize the various sources of systematics, are described in the text.
%
...
listed in Tab.~\ref{tab:unpolLQCDstatus1} with two or more sea 
quark flavors.
%
...
Tabs.~\ref{tab:polLQCDstatus1B}-\ref{tab:polLQCDstatus2B}, for completeness.
...
%
Note that tolerance of $\Delta chi^2=1$ at 68\% CL is used in the CJ15 PDF set; 
hence, the smaller uncertainties of this set compared to all the other PDF sets.
%
...
The moments in Tab.~\ref{tab:polPDFmoms} therefore hardly differ
in the two analyses.} and JAM15~\cite{Sato:2016tuz}.
%
The most relevant features of these PDF sets have been presented in
...
polarized PDFs listed in Tab.~\ref{tab:polPDFmoms}, we apply an unweighted 
...
are summarized in Tab.~\ref{tab:BMunp}.
%
...
(left panel) and of ratios to the lattice values (right panel).
...
As is apparent from Tab.~\ref{tab:BMunp} and Fig.~\ref{fig:Bmomsunp}, there is 
...
The benchmark values of the first moments of the unpolarized PDFs, obtained
as described in Sects.~\ref{subsubsec:BClQCD}-\ref{subsubsec:GPDFfits}, 
are summarized in Tab.~\ref{tab:BMpol}.
%
...
$N_f=2$ sea quarks from lattice QCD, and the two values from the 
...
%
In contrast with the NNPDFpol1.1, DSSV08 and JAM15 fits, in the JAM17 result 
...
\bottomrule
\end{tabular}
\caption{\small Same as Tab.~\ref{tab:BMunp}, but for the polarized benchmark 
moments.}
\label{tab:BMpol}
\end{table}
%-------------------------------------------------------------------------------
...
As is apparent from Tab.~\ref{tab:BMpol} and Fig.~\ref{fig:Bmomspol}, the 
size of the uncertainties on the moments is in general comparable between the 
lattice-QCD and the global-fit results, in contrast  with the unpolarized case.
%
