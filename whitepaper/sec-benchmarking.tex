...
sources of uncertainty, are denoted with superscript~$\dagger$.
%
...
the uncertainties on the moments is in general comparable between the lattice 
QCD and the global fit results, at variance with the unpolarised case.
%
...
the uncertanty on the lattice QCD result is comparable to or smaller than the 
uncertainty on the global fit result.
%
This is particularly remarkable for the first moments of the total down and 
strange quark distributions.
%
In this case, the lattice QCD and teh global fit results are discrepant
within each other uncertainties by two$\sigma$ at most (in units of the
lattice QCD uncertainty).
%
On the one hand, we observe that the uncertainty on th elattice QCD results 
...
by requiring SU(3) symmetry, {\i.e.} by imposing in the fit the experimental
value (with a possibly inflated uncertainty) of the octet PDF combination,
as explained in Sec.~\ref{sec:polPDFs}.
%
...
affect the resulting polarised PDFs.
...
All these remarks still hold if individual lattice-QCD and/or global-fit
results are in Tabs.~\ref{tab:gAstatus}-~\ref{tab:polLQCDstatus0}-\ref{tab:polLQCDstatus1}-\ref{tab:polPDFmoms}
...
input to global fit of polarised PDFs, especially in determining the
extrapolation in the completely unknown small-$x$ region.
%
